...
| 20.54 | Agent platform (MS agents-for-beginners) | 20.28 | implemented | `src/agent-os/agent-platform/` (manifests, 10 patterns, R0–R4, secure envelope) | **typecheck errors in `service.ts`** | — | **this-run typecheck: FAIL** | IMPLEMENTED (defects open) |
| 20.55 | ARTEMIS → Pao mobile runtime | 20.12b | implemented | Flash/Pro router, device leases, `pao.mobile.*` | — | — | owner-doc | IMPLEMENTED |
| 20.56 | Micro-app capability lab (qxresearch) | 20.28 | implemented | `src/agent-os/capability-lab/` + tests | **typecheck errors in `service.ts`**; follow-ups 20.56.1–.6 not implemented | — | **this-run typecheck: FAIL** | IMPLEMENTED (defects open) |
| 20.57 | SkillsGate skill control plane | 08,20.25 | **in-flight / unverifiable** | `src/agent-os/skill-gate/` (10 files, rewritten in tree) | recon-planned `service.ts`/`mcp-tools.ts`/`skill-gate-routes.ts` missing; no report ever written; **typecheck error in `store.ts`**; impl commit only on backup branch | — | **this-run typecheck: FAIL; no report** | BLOCKED (in-flight) |
| 20.58 | Security plane | 20.28 | **absent from working tree** | commit `e2f5e62af` on `backup/admiring-noyce-main-based` only (`src/security/` 16 files + tests) | whole module absent from current checkout; no docs | — | commit-message only | NOT_STARTED (in current tree) |
| 20.59 | Credential runtime | 20.58 | **absent from working tree** | same commit (`src/credentials/` 19 files + tests) | whole module absent from current checkout; no docs | — | commit-message only | NOT_STARTED (in current tree) |
...
| 20.64–20.80 | (blueprints in Downloads, not in repo) | — | **20.64 = scaffold only, unwired**: `src/agent-os/visual-compute/` (7 files: config/docs/policy/runtimes/store/types/wgsl) has **zero** routes/MCP/tests/importers — dead code today; 1 typecheck error in `runtimes.ts` | — | full runtime per blueprints (routes, MCP tools, dashboards, tests) | — | **this-run: unwired + typecheck FAIL (1 file)** | PARTIAL (scaffold, 20.64 only); 20.65–20.80 NOT_STARTED in repo |
| 20.81 | OpenCodeReview runtime | 20.80 | not in tree | — | deterministic review runtime per blueprint | — | — | NOT_STARTED |
