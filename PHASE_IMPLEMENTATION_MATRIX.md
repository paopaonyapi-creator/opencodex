...
| 21.03 | CC-Switch and Navop Runtime | 20.98-21.02 | **partial: host runtime and provider registry verified; failover slice added. Full phase still open: real CLI adapters, config projection/rollback, credential broker, MCP/skills/prompt/session federation, budget guard** | src/agent-os/navop/, schema v75 (ccs_routes, ccs_circuit_breakers, ccs_usage_events), REST /api/agent-os/navop/*, MCP pao.* | Codex/Claude projection, credential broker, real gateway | — | **this-run: 10/10 pass in tests/navop.test.ts; bun x tsc --noEmit pass. Not full-suite or live-provider proof** | PARTIAL |
