...
| 20.82 | CortexKit AFT sensorimotor runtime | 20.80, 20.81 | **implemented & verified (transactional loop)** | `src/agent-os/sensorimotor/` (types, perception, actions, service, mcp-tools, index), schema v55 (`sm_sessions`, `sm_perceptions`, `sm_actions`, `sm_observations`, `sm_checkpoints`, `sm_audit`), REST `/api/agent-os/sensorimotor/*`, MCP `pao.aft.*` (health/perceive/act) | LSP-grade symbol resolution (pinned AFT toolchain), PTY streaming | — | **this-run: 19 pass (unit + routes)** — policy deny-by-default, approval gate, checkpoint/rollback, timeout→timed_out, abort, sandbox traversal block; fixed runner `timedOut` flag bug | VERIFIED |
| 20.84 | TypeSafe Jev Decision Intelligence | 20.85 | **implemented & verified (engine, calibration, fusion, audit)** | `src/agent-os/decision/` (contracts, calibration, fusion, engine, types), schema v54 (`dec_*`), 10-stage policy fusion | TypeSafe live early-access API | — | **this-run: 7/7 pass** | VERIFIED |
| 20.85 | OmniRoute Unified AI Gateway | 20.84, 20.72 | **implemented & verified (gateway, budget, routing, circuit, CLI)** | `src/agent-os/model-gateway/` (registry, envelope, budget, circuits, direct/omniroute adapters), schema v54 (`gw_*`), CLI `ocx gateway`, GUI `ModelGateway.tsx` | live remote OmniRoute daemon (Direct fallback active) | — | **this-run: 8/8 pass** | VERIFIED |
