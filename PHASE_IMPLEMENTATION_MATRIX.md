...
| 20.96 | MCP Fabric — AnythingMCP capability control plane | 20.57, 20.85 | **implemented & verified** | `src/agent-os/mcp-fabric/`, schema v67 (`amf_*`), REST `/api/agent-os/mcp-fabric/*`, MCP `pao.connector.*`, docs `docs/PHASE_20.96_MCP_FABRIC.md` | live AnythingMCP execute discovery-gated | — | **this-run: 24/24 pass** | VERIFIED |
| 20.98 | OpenHermit — Durable Multi-Agent Fleet Runtime | 20.61, 20.57, 20.85, 20.96 | **implemented & verified (adapter-first, state plane, sandbox, skills/MCP sync, deep research, approvals)** | `src/agent-os/openhermit/`, schema v68 (`oh_*` 14 tables), REST `/api/agent-os/openhermit/*`, MCP `pao.openhermit.*`, docs `docs/Phase_20.98_Pao-hubPro_x_OpenHermit.md` | live external OpenHermit gateway deployment (tested via deterministic FakeHermitProvider & HTTP provider contract) | — | **this-run: 36/36 pass across unit, contract, policy, security, recovery, resilience** | VERIFIED |
| 20.99 | Whip — Mobile Agent Operations Console | 20.61, 20.85, 20.98 | **implemented & verified (clean-room architecture: SSH/Tailscale host fabric, strict host-key verify, unified fleet, transcripts, terminals, SFTP workspace, offline queue, QR pairing, R0-R4 approvals)** | `src/agent-os/whip/`, schema v69 (`whip_*` 9 tables), REST `/api/agent-os/whip/*`, MCP `pao.whip.*`, docs `docs/Phase_20.99_Pao-hubPro_x_Whip.md` | downstream native mobile app store distribution | — | **this-run: 17/17 pass across unit, transport, transcript, terminal, files, queue, pairing, approvals** | VERIFIED |
| 21.00 | Unified Agent Operations Control Plane | 20.98, 20.99 | **implemented & verified (cross-host runtime federation, durable jobs, central MCP/skill governance, approval fabric, audit stream, idempotency, quarantine, 18 mandatory security invariants)** | `src/agent-os/control-plane/` (unified-service, unified-store, unified-types, unified-mcp-tools), schema v70 (`uap_*` 9 tables), REST `/api/agent-os/unified/*`, MCP `pao.uap.*`, docs `docs/Phase_21.00_Pao-hubPro_Unified_Agent_Operations_Control_Plane.md` | downstream web operations console GUI pages | — | **this-run: 24 pass across 2 test suites (13 core + 11 security invariants); overall regression baseline: 107/107 pass across 7 test suites** | VERIFIED |
| 21.01 | Parley — Multi-Agent Work Room | 20.98, 20.99, 21.00 | **implemented & verified (multi-agent addressing @codex/@claude/@both, runtime identity badge, Run Inspector timeline, 4-level tool policy, handoff chain, transcript export)** | `src/agent-os/parley/` (room-engine, store, types, flags, mcp-tools), schema v71 (`parley_*` 8 tables), REST `/api/agent-os/parley/*`, MCP `pao.parley.*`, docs `docs/Phase_21.01_Pao-hubPro_Parley_Multi-Agent_Work_Room.md` | web UI work room drawer | — | **this-run: 8/8 pass across room engine, addressing, tool classification, inspector, handoff, markdown export; overall regression baseline: 115/115 pass across 8 test suites** | VERIFIED |
| 21.02 | Multi-Agent Mission Control | 20.98, 20.99, 21.00, 21.01 | **implemented & verified (fleet overview, run state machine & timeline, DLQ routing, human approval inbox, incident mode, emergency stop, audit ledger, MCP tools & REST APIs)** | `src/agent-os/mission-control/` (service, store, types, flags, mcp-tools), schema v72 (`mc_*` 8 tables), REST `/api/agent-os/mission-control/*`, MCP `pao.mission_control.*`, docs `docs/Phase_21.02_Pao-hubPro_Multi-Agent_Mission_Control.md` | mission control web dashboard | — | **this-run: 9/9 pass across agent supervision, run lifecycle, DLQ, approval inbox, emergency lockdown, audit ledger, MCP tools; overall regression baseline: 124/124 pass across 9 test suites** | VERIFIED |
| 21.03 | CC-Switch & Navop Runtime | 20.98–21.02 | **implemented & verified (host-authoritative operations runtime, resource registry, session broker, risk-based execution router R0..R5, human approval gate, CC-Switch cross-CLI provider & runtime federation, pao.* MCP tools & REST APIs)** | `src/agent-os/navop/` (service, store, types, flags, mcp-tools), schema v73 (`navop_*` & `ccs_*` 9 tables), REST `/api/agent-os/navop/*`, MCP `pao.*` (`pao.resources.*`, `pao.capabilities.*`, `pao.execution.*`), docs `docs/PHASE_LATEST_PAO_HUBPRO_NAVOP_ARCHITECTURE.md`, `docs/Phase_21.03_Pao-hubPro_x_CC-Switch.md` | host resources & provider cockpit | — | **this-run: 8/8 pass across resource registry, path-traversal guard, R4 approval gate, CC-Switch provider federation, MCP tools; overall regression baseline: 115/115 pass across 9 test suites** | VERIFIED |
| 21.02-H3 | MiniMax H3 Extender Video Plane | 20.92, 21.02 | **implemented & verified (persistent long-form video projects, scene clips, attempts, generation hash, validation/invalidation engine, h3_extender_jobs, MCP tools & REST APIs)** | `src/agent-os/h3-extender/` (service, store, types, flags, mcp-tools), schema v74 (`h3_*` 10 tables), REST `/api/video/h3/*`, MCP `pao.video.h3.*`, docs `docs/PHASE_21.02_PAO_HUBPRO_MINIMAX_H3_EXTENDER.md` | video execution plane | — | **this-run: 5/5 pass across project creation, clip planning, generation attempts, prompt mutation invalidation, MCP tools; overall regression baseline: 120/120 pass across 10 test suites** | VERIFIED |
