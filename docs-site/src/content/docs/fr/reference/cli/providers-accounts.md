...
où l'ordre ne s'applique pas, comme les comptes OAuth et les clés API. Avec au moins deux comptes Kiro enregistrés et éligibles, une réponse 429 entraîne automatiquement une rotation vers un autre
compte, en privilégiant celui dont l'allocation restante connue est la plus élevée ; `ocx account login kiro` ajoute les comptes au pool un par un. Un résultat vide est toujours un succès. `--json`
renvoie :
