...
| **TypeSafe Jev Real Transport (20.84)** | `src/agent-os/decision/provider-mode.ts` | Real live HTTP transport, official schema `typesafe-jev-systemone@1.13.0`, token cost accounting | `tests/typesafe-jev-real.test.ts` (3/3 pass) | **VERIFIED** |
| **ZCode Workspace Runtime (20.100)** | `src/agent-os/zcode/` | ZCode Runtime Adapter, Provider Lease Bridge, 10-stage Permission Bridge, Event Journal Replay, Reviewer Council Gate, MCP/Skills Projection | `tests/zcode-runtime.test.ts`, `tests/zcode-browser-routes.test.ts` (15/15 pass) | **VERIFIED** |
| **Universal Browser Control Plane (20.101)** | `src/agent-os/browser-control/` | Local Chrome & Oya Browser adapters, Universal Browser Router, Persona & Session Vault, Playbook Engine, Drift Repair, Human Takeover | `tests/browser-control-plane.test.ts`, `tests/zcode-browser-routes.test.ts` (17/17 pass) | **VERIFIED** |
