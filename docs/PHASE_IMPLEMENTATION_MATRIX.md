...
| **20.64** | Vercel vgpu Visual Compute | WebGPU & Headless Shader Runtime | 19, 20 | `src/agent-os/visual-compute/` | **VERIFIED** | 3 focused tests pass (this run) | Sandbox execution, no host GPU leak | Visual compute engine for 20.7 & 20.31 media |
...
| **20.82** | CortexKit AFT Sensorimotor IDE | Agent-Native IDE, Symbol Perception & PTY | 20.61, 20.81 | `src/agent-os/sensorimotor/` | **VERIFIED** | 19 tests pass (unit + routes, this run) | Transactional checkpoint/rollback, deny-by-default policy, approval gate, timeout/abort | Perception→plan→act→observe loop; schema v55 `sm_*`; REST `/api/agent-os/sensorimotor/*`; MCP `pao.aft.*` |
