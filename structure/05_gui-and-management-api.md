...
| Providers | Create/update/delete ordinary provider configs and enrich registry metadata. `POST /api/providers/switch-pool` preflights a replacement key-auth endpoint and default model before atomically replacing that provider's endpoint and complete key pool; failures and concurrent edits commit nothing, while success clears the old endpoint-scoped model allowlist before catalog convergence. The reserved `openai` card exposes Pool(default)/Direct account mode; `openai-apikey` remains the separate API route. |
...
| Provider quotas and tests | `src/server/management/provider-routes.ts` — `GET /api/provider-quotas`, `POST /api/providers/test`, `POST /api/providers/switch-pool`, `GET/PUT /api/provider-context-caps`, `GET /api/provider-presets`. A quota read may be served from cache or force-refreshed; absent quota data is reported as unknown rather than as a measured zero. Pool switching accepts the new credential only on the management body/CLI stdin path, returns no secret, requires a verified live model roster containing the requested default, replaces old endpoint-bound fallback keys, then requests catalog convergence. |
