...
describe("non-OpenAI tool catalog nudge", () => {
  ...
  test("injects contextual patch guidance only for declared nested helpers", () => {
    ...
    const applyPatchOnly = buildNonOpenAIToolCatalogNudgeForTools([exec("declare const tools: { apply_patch(input: string): Promise<unknown> }")]);
    expect(applyPatchOnly).toContain("targeted code edits");
    // The generic code-mode orientation paragraph names exec_command only as a nested-call
    // syntax example; the contextual patch guidance itself must stay scoped to declared helpers.
    expect(applyPatchOnly?.split("For targeted code edits")[1]).not.toContain("exec_command");
  });
